...
const handleAddOrUpdate = async () => {
  ...
  const addTask = async () => {
    await addDoc(collection(db, 'Policy'), {
      title: item,
      description,
  ...
  const updateTask = async () => {
    const taskDocRef = doc(db, 'Policy', updatingTaskId);
  ...
  const handleDelete = async (taskId) => {
    try {
      ...
      const taskDocRef = doc(db, 'Policy', taskId);
  ...
  useEffect(() => {
    const q = query(collection(db, 'Policy'), orderBy('created', 'desc'));
      ...
      <div>
        ...
        {/* <img src={MyImage} alt="" style={{height:600,width:800}}/> */}
          <div style={{ width: '500px', padding: '20px',border:"3px solid", borderRadius: '8px', backgroundColor: '#fff', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', borderColor:"rgb(166, 233, 139)"}}>
            <h2 style={{ textAlign: 'center', color: '#333' }}>{updatingTaskId ? 'Update' : 'Add'} Privacy Policy</h2>
